...
object Unit06 {
  ...
  @main def exercise62(): Unit =
    ...
    println()
    println(descending.above(descending.reflectH))
    println()
    println(descending.+(descending.reflectV).above(descending.reflectH.+(descending.reflectH.reflectV)))
    println()
    println(ascending.above(ascending.reflectH).+(ascending.reflectV.above(ascending.reflectV.reflectH)))
    println()
    println(descending.reflectH.above(descending).+(descending.reflectV.reflectH.above(descending.reflectV)))
    println()
  ...
  @main def exercise63(): Unit =
    ...
    val paraUpper = flow(width = 40, words.toUpperCase)
    println(para.+(paraUpper).frameBNC)
    println(para.borderR().+(paraUpper.borderL().frameL).frameBNC)
  ...
  case class InfRule(premises: Seq[String], conclusion: String, name: String = "") {
    ...
    def toPicture: Picture = Picture(premises.mkString(", ")).frameB.borderR().^(Picture(conclusion), MID).+(Picture(name), MID)
  }
  ...
  @main def exercise64(): Unit =
    ...
    val pic = Picture("1").borderR().+(Picture(modusPonens.toString).borderL(), TOP).^(Picture("2").borderR().+(Picture(orElim.toString).borderL(), TOP)).^(Picture("3").borderR().+(Picture(reductio.toString).borderL(), TOP)).^(Picture("4").borderR().+(Picture(deduction.toString).borderL(), TOP))
    println(modusPonens)
    println(orElim)
    println(reductio)
    println(deduction)
  ...
  @main def exercise65(): Unit =
    ...
    val bulletPoints: Vector[Picture] = paras.zipWithIndex map ((p, i) => (Picture(s" (${i + 1}) ") + p).borderT())
